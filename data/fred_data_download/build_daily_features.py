...
def _calendar_from_file(path: Path, date_column: str) -> pd.DatetimeIndex:
    ...
    if date_column in df.columns:
        ...
        date_values = df.index.to_series(index=df.index)
    else:
        raise ValueError(f"Calendar date column {date_column!r} not found in {path}")

    dates = pd.to_datetime(date_values).dropna().drop_duplicates().sort_values()
...
def main() -> None:
    ...
    out = EXPERIMENTS_DIR / "fred_daily_features.parquet"
