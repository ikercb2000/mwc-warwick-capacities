...
losses = dataset["portfolio_loss_1d"].dropna()
adf = adfuller(losses.to_numpy(), autolag="AIC")
adf_table = pd.Series(
    {
        "ADF statistic": adf[0],
        "p-value": adf[1],
...
hac = hac_model_comparison(
    actual_test,
    result.predictions,
    reference="Choquet AR",
    loss="squared",
    max_lags=10,
)
vix = dataset["vix"].reindex(result.predictions.index)
high_vix = vix >= vix.quantile(0.90)
...
plot_acf(losses, lags=30, ax=axes[1])
plot_pacf(losses, lags=30, ax=axes[2], method="ywm")
figure.tight_layout()
...
errors.rolling(60).std().plot(
    ax=axes[1], title="Rolling 60-day forecast-error volatility"
)
for axis in axes:
    axis.grid(alpha=0.2)
figure.tight_layout()
